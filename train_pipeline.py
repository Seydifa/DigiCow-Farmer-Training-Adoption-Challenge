...
# Import project modules from scripts directory
...
from scripts.utils.data_utils import load_data, extract_topics_vectorized, reduce_memory_usage
from scripts.feature_engineering import FeatureEngineer
from scripts.model_config import BASE_MODELS, ADVANCED_MODELS, MODEL_PRESETS, HYPERPARAMETER_SPACES
from scripts.ensemble_pipeline import EnsembleModelPipeline
from scripts.utils.device_utils import is_gpu_available
from scripts.utils.printing_utils import print_header, print_dataframe, print_success, print_warning
...
def train_models(
    ...
    if optimize:
        if use_optuna:
            logger.info(f"\nPerforming Advanced Bayesian Optimization (Optuna) with {n_trials} trials...")
            from scripts.optimization import OptunaTuner
            
            # Initialize tuner
            tuner = OptunaTuner(n_trials=n_trials, cv=3, random_state=RANDOM_SEED, n_jobs=-1)
            
            # Tune each model
            optimized_models = {}
            for name in pipeline.base_models:
                try:
                    best_model = tuner.tune(name, X_train, y_train)
                    optimized_models[name] = best_model
                except Exception as e:
                    logger.error(f"Optuna tuning failed for {name}: {e}")
                    optimized_models[name] = pipeline.base_models[name]
            
            pipeline.base_models = optimized_models
            logger.info("Optuna optimization completed.")
            
        else:
            logger.info("\nPerforming hyperparameter optimization (RandomizedSearchCV)...")
    ...
    if calibrate:
        ...
        pipeline.calibrate_base_models(cv=3, method='isotonic')
    ...
    # Display results
    print_dataframe(results_df, title="MODEL EVALUATION RESULTS")
...
def main(
    ...
    logger.info(f"Optimize: {optimize} (Optuna: {use_optuna}, Trials: {n_trials})")
    logger.info(f"Calibrate: {calibrate}")
    logger.info(f"GPU Accelerated: {is_gpu_available()}")
    ...
    try:
        ...
        pipeline, results_df = train_models(
            X_train, y_train, 
            model_preset, 
            optimize=optimize,
            calibrate=calibrate,
        ...
        test_ids = test_df['ID']
        # Save submissions to dedicated submissions folder in project root
        project_root = Path(__file__).parent.parent
        submissions_dir = project_root / "submissions" / f"run_{timestamp}"
        predictions = generate_predictions(ensembles, pipeline, X_test, test_ids, submissions_dir)
        
        # Save models and results to data/processed
        ...
        logger.info(f"Submissions saved to: {submissions_dir}")
...
def generate_predictions(
    ...
    """
    Generate predictions from all ensembles in competition format.
    
    Competition format requires:
    - ID column
    - Target_AUC column (probability predictions)
    - Target_LogLoss column (probability predictions)
    ...
    for name, ensemble in ensembles.items():
        ...
        # Ensure predictions are valid probabilities [0, 1]
        predictions = np.clip(predictions, 0.0, 1.0)
        
        all_predictions[name] = predictions
        
        # Save individual submission in competition format
        # Both Target_AUC and Target_LogLoss use the same probability predictions
        submission = pd.DataFrame({
            'ID': test_ids,
            'Target_AUC': predictions,
            'Target_LogLoss': predictions
        })
    ...
    # Ensure meta predictions are also clipped
    meta_predictions = np.clip(meta_predictions, 0.0, 1.0)
    
    submission = pd.DataFrame({
        'ID': test_ids,
        'Target_AUC': meta_predictions,
        'Target_LogLoss': meta_predictions
    })
    filepath = output_dir / "submission_meta_ensemble.csv"
    submission.to_csv(filepath, index=False)
    logger.info(f"  Saved: {filepath}")
    logger.info(f"  Probability range: [{meta_predictions.min():.4f}, {meta_predictions.max():.4f}]")
    
    logger.info(f"\n✓ All predictions saved to {output_dir}")
    logger.info(f"\n📊 Submission Format:")
    logger.info(f"  - ID: Test sample identifier")
    logger.info(f"  - Target_AUC: Probability predictions for ROC-AUC metric (30% weight)")
    logger.info(f"  - Target_LogLoss: Probability predictions for Log Loss metric (70% weight)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--calibrate',
        action='store_true',
        help='Perform probability calibration'
    )
    parser.add_argument(
        '--optuna',
        action='store_true',
        help='Use Optuna for optimization (requires --optimize)'
    )
